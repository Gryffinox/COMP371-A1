...
public:
    ...
    Model(const char* filepath)
    {
        ...
        // read file's buffer contents into strea
        // close file handlers
        modelFile.close();
        ...
        for (int i = 0; i <numCubes; i++)
        {
            ...
            x = std::stof(token.substr(start, end));
            token = token.substr(end+1);
            end = token.find(",");
            y = std::stof(token.substr(start, end));
            token = token.substr(end+1);
            end = token.find(",");
            z = std::stof(token.substr(start, end));
            token = token.substr(end+1);
            
            originalModel[i] = glm::vec3(x,y,z);
        }
    ...
    void setPositionArrays(glm::vec3* originalModel)
    {
